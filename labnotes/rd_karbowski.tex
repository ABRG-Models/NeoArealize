...
\mb{J}_i(\mb{x},t) = D \nabla \big(a_i(\mb{x},t) - \frac{D'}{N-1} \textstyle \sum_{p\ne i}^N a_p(\mb{x},t) \big) - a_i
\sum_{j=1}^M \big(\gamma_{i,j} \nabla\rho_j(\mb{x}) \big)
\end{equation}
%
...
current. The `positive' diffusion is $D$, and the `negative' diffusion
is $\frac{D D'}{N-1}$. This successfully produced competition between
$N$=2 TC types in the absence of any guidance molecules with $D$=0.1
and $D'$ set to a value of 0.01.
