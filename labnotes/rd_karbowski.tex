...
reintroduce the additional diffusion ideas from
sections \ref{sec:comp2} (flux current affected by gradient of other
branching densities); \ref{sec:comp3} (diffusion is increased into
regions with low $n$); and \ref{sec:comp7} (multiplicative reduction
of diffusion of $a_i$).

Note that the mechanism from section \ref{sec:comp2} shouldn't need
the sigmoid, as $a_i$ is already divisively normalized in this scheme.
...
\ref{sec:comp3} (diffusion is increased into
regions with low $n$); and \ref{sec:comp7} (multiplicative reduction
of diffusion of $a_i$).
