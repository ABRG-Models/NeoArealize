...
I think I like the idea of computing $\partial c_i/\partial t$ first
in the simulation, then using that as one part of the $\partial
a_i/\partial t$ computation. Also, I think there should be an
additional `branching accumulation' term to reflect the idea that
branching increases with time.

I've tried these ideas. They again suffer from the problem that
there's no upper bound on $a$ (even with the divisive normalization).
...
%
\begin{equation} \label{eq:Karb2D_dc_comp10}
\frac{\partial c_i}{\partial t} = -\alpha_i c_i
+ \beta_i n
[a_i]^k
\end{equation}
%
\begin{equation} \label{eq:Karb2D_da_comp10}
\frac{\partial a_i}{\partial t}
= \nabla\vcdot\mb{J}_i + \alpha_i c_i - \beta_i n [a_i]^k
\end{equation}
%
\begin{equation}\label{eq:norm_comp1}
a_i' = a_i \quad or \quad a_{max}
\end{equation}
%
Fix the maths above!
($a_i'$ becomes $a_i$ in the next computational step.)
%
\begin{equation} \label{eq:Karb2D_conserve_NM_comp10}
n = 1 - \sum_{i=1}^{N} c_i
\end{equation}
%
