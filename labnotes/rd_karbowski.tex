...
\end{equation}
%
is not a function of time. To show this, start by adding
Eqs.~\ref{eq:Karb1D_dc} and \ref{eq:Karb1D_da} to give
%
\begin{equation}
...
\mb{J}_i(\mb{x},t) = \nabla \big(D a_i(\mb{x},t) - \frac{F}{N-1} \textstyle \sum_{p\ne i}^N a_p(\mb{x},t) \big) - a_i
\sum_{j=1}^M \big(\gamma_{i,j} \nabla\rho_j(\mb{x}) \big)
\end{equation}
%
which provides $F$ as a parameter to define the strength of the
contribution of the non-self axon branching densities to the flux
current. The diffusion constant is $D$, and the `negative'
diffusion is $\frac{F}{N-1}$. This successfully produced competition
between $N$=2 TC types in the absence of any guidance molecules with
$D$=0.1 and $F$ set to a value of 0.001.

\textbf{Stop press} Actually, no, this doesn't do anything interesting
at all, now that I've fixed the bugs. If $D=F$ for a
\code{2N0M} system, then the system is static. If $F$>$D$, then the
...
Now implement this:
%
\begin{equation} \label{eq:Karb2D_J_NM_with_comp4}
\mb{J}_i = D \nabla a_i
-a_i E \nabla n
+a_i F \nabla \hat{a}_i
 - a_i \mb{g}_i
\end{equation}
%
which should result in the following for the divergence of $\mb{J}_i$:
%
...
& \quad - E a_i \dvrg n - E \nabla n \vcdot \nabla a_i
%
- F a_i \dvrg \hat{a}_i - F \nabla \hat{a}_i \vcdot \nabla a_i \\
%
& \quad - a_i\nabla\vcdot\mb{g}_i - \mb{g}_i\vcdot\nabla a_i
\end{split}
\end{equation}

Values for $F$ between 0.01 and -0.006 make small differences to the
result, but outside this range, the simulation becomes unstable.
...
%
\begin{equation} \label{eq:Karb2D_dc_comp5}
\frac{\partial c_i}{\partial t} = -\alpha_i c_i
+ \beta_i n
[a_i]^k
\end{equation}
%
\begin{equation} \label{eq:Karb2D_da_comp5}
\frac{\partial a_i}{\partial t}
= \nabla\vcdot\mb{J}_i + \alpha_i c_i - \beta_i n
[a_i]^k - \frac{\epsilon_i}{N-1} a_i \sum_{p \ne i}^{N} a_p^l
\end{equation}
%
\begin{equation} \label{eq:Karb2D_conserve_NM_comp5}
n = 1 - \sum_{i=1}^{N} c_i(\mb{x}, t)
\end{equation}
%
\begin{equation} \label{eq:divJExpanded_comp5}
%
\nabla\vcdot\mb{J}_i = D \dvrg a_i
%
- E a_i \dvrg n
...
Note that
Eqs.~\ref{eq:Karb2D_dc_comp5}--\ref{eq:Karb2D_conserve_NM_comp5} are
...
\subsection{Axon guidance reading}

Erzurumlu \& Gaspar (cite) have a great table of chronological events
in the development of the whisker barrels.
...
At around birth (E19 to P0), barralette patterns emerge in principle
