...
molecule is expressed. Axon branching should diffuse into areas with
low connections, so follow the gradient of this `connectivity
molecule' from high $n$ to low $n$:
%
\begin{equation} \label{eq:Karb2D_J_NM_with_comp3}
\mb{J}_i(\mb{x},t) = \nabla \big(D a_i(\mb{x},t)
- \frac{D'}{N-1} \textstyle \sum_{p\ne i}^N a_p(\mb{x},t) - D^n n(\mb{x}, t) \big) - a_i
\sum_{j=1}^M \big(\gamma_{i,j} \nabla\rho_j(\mb{x}) \big)
\end{equation}
%
With the new term, it's probably worth going back to look at the
method for computing $\nabla\vcdot\mb{J}_i$. Let's first take the case
for which $D'$=0, and also write down that
%
\begin{equation} \label{eq:g_i}
\mb{g}_i(\mb{x}) = \sum_{j=1}^M \big(\gamma_{i,j} \nabla\rho_j(\mb{x}) \big)
\end{equation}
%
giving this (less gnarly) expression for $\mb{J}_i$, in which I've dropped
references to the arguments ($\mb{g}_i(\mb{x})$ becomes $\mb{g}_i$,
$n(\mb{x}, t)$ becomes $n$, and so on):
%
\begin{equation} \label{eq:Karb2D_J_NM_with_comp3_Dprime0}
\mb{J}_i = \nabla \big(D a_i - D^n n \big) - a_i \mb{g}_i
\end{equation}
%
and the divergence of $\mb{J}_i$ is
%%%%
\begin{equation}
\nabla\vcdot\mb{J}_i = \nabla\vcdot \big[ \nabla \big(D a_i - D^n n \big) - a_i \mb{g}_i \big]
\end{equation}
%
Because the divergence operator is distributive
%
\begin{equation}
\nabla\vcdot\mb{J}_i = D \dvrg a_i - D^n \dvrg n
...
\begin{equation}
%
\nabla\vcdot\mb{J}_i = D \dvrg a_i - D^n \dvrg n - \big(
a_i\nabla\vcdot\mb{g}_i
+
\mb{g}_i\vcdot\nabla a_i
\big)
...
\end{equation}
%
Finally, let the sum of the branching of all the \emph{other} thalamocortical
axon types be
%
\begin{equation}
...
\end{equation}
%
and consider the case that $D'$>0.
%
\begin{equation} \label{eq:Karb2D_J_NM_with_comp3_2}
\mb{J}_i = \nabla \big(D a_i - \frac{D'}{N-1} \hat{a}_i - D^n n \big) - a_i \mb{g}_i
\end{equation}
%
\begin{equation}
\begin{split}
\nabla\vcdot\mb{J}_i & = \nabla\vcdot \big[ \nabla \big(D a_i
- \frac{D'}{N-1} \hat{a}_i - D^n n \big) - a_i \mb{g}_i \big] \\
%
& = D \dvrg a_i - \frac{D'}{N-1} \dvrg \hat{a}_i - D^n \dvrg n
- \nabla\vcdot a_i \mb{g}_i \\
%
& = D \dvrg a_i - \frac{D'}{N-1} \dvrg \hat{a}_i - D^n \dvrg n
- \big(
...
D \dvrg a_i % term1
-
\frac{D'}{N-1} \dvrg \hat{a}_i % term 1.2
-
D^n \dvrg n % term 1.5
-
a_i\nabla\vcdot\mb{g}_i % term2
...
\end{equation}
%
Because the divergence is distributive, we can equally write this as
%
\begin{equation}
\nabla\vcdot\mb{J}_i  =
\dvrg \big[
...
\end{equation}
%
summing $D a_i$, $\frac{D'}{N-1}\hat{a}_i$ and $D^n n$ before
computing the divergence using the relation in
Eq.~\ref{eq:computeDivJTerm1}.
