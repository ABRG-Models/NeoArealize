...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Upper bound on $a_i$ and $\hat{a}_i$ inhibits $\dot{a}_i$}
\label{sec:comp10}

This is some sort of fixed upper limit either on the branching density
per element, of of the total branching density within a single TC type.

One option is to let $n_c \equiv n$ and introduce a new $n_a$ such that
%
\begin{equation}
n_c = 1 - \sum_i c_i
\end{equation}
...
The alternative is to put a sigmoid on each $a_i$, so that it
saturates at a finite value. Again, this could be argued to result
from physical or chemical constraints.
