...
int main (int argc, char **argv)
{
    ...
    while (doing) {
        ...
        if ((RD.stepCount % plotevery) == 0) {
            plt.scalarfields (displays[0], RD.hg, RD.n);
            plt.scalarfields (displays[1], RD.hg, RD.c);
            if (vidframes) {
                plt.savePngs (logpath, "n", framecount, displays[0]);
                plt.savePngs (logpath, "c", framecount, displays[1]);
                ++framecount;
            } else {
                plt.savePngs (logpath, "n", RD.stepCount, displays[0]);
                plt.savePngs (logpath, "c", RD.stepCount, displays[1]);
            }
        }
#endif
