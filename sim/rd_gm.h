...
/*!
 * Two component Gierer-Meinhardt Reaction Diffusion system
 *
 * Fixme: There's duplicated code here and in RD_Schnakenberg. These
 * could have an RD_Turing base class.
 */
template <class Flt>
class RD_GM : public RD_Base<Flt>
{
public:
    /*!
     * Reactant A
...
#pragma omp parallel for
        for (unsigned int h=0; h<this->nhex; ++h) {
            dAdt[h] = this->k1 - (this->k2 * A_[h]) + (this->k3 * A_[h] * A_[h] / this->B[h]) + (this->D_A * lapA[h]);
        }
    }
...
#pragma omp parallel for
        for (unsigned int h=0; h<this->nhex; ++h) {
            // G = k4 A^2       - k5 A^2 B
            dBdt[h] = (this->k4 * this->A[h] * this->A[h]) - (this->k5 * B_[h]) + (this->D_B * lapB[h]);
        }
    }
    //@}
