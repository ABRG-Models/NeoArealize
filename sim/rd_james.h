...
// Number of ThalCort populations
#ifndef N_TC
# error "You must define N_TC, the number of thalamocortical projections. Would like this to be runtime settable actually."
#endif
...
public:
    ...
    void init (vector<morph::Gdisplay>& displays) {
        ...
        // Make one axon branching decay parameter stronger
        this->beta[0] = 5;
        ...
        } else if (this->rhoMethod == GuidanceMoleculeMethod::TwoDWaves) {
            // Construct 2 dimensional gradents
            DBG ("2D Chemo attractants!");
...
#elif defined TWO_GUIDES
                this->g[i][0][h.vi] = (this->gammaA[i] * this->grad_rhoA[0][h.vi]
                                       + this->gammaB[i] * this->grad_rhoB[0][h.vi]) * bSig;
                this->g[i][1][h.vi] = (this->gammaA[i] * this->grad_rhoA[1][h.vi]
                                       + this->gammaB[i] * this->grad_rhoB[1][h.vi]) * bSig;
#else // if defined ONE_GUIDE
                this->g[i][0][h.vi] = (this->gammaA[i] * this->grad_rhoA[0][h.vi]) * bSig;
                this->g[i][1][h.vi] = (this->gammaA[i] * this->grad_rhoA[1][h.vi]) * bSig;
#endif
            }
        }
    }
...
#pragma omp parallel for schedule(dynamic,50)
    ...
    void gaussian1D_guidance (void) {
        ...
        for (auto h : this->hg->hexen) {
            ...
            this->rhoA[h.vi] = gain_smoid / (1.0 + exp(-(x_-xoffA)/sigma));
    ...
    void gaussian2D_guidance (void) {
        // Potentially alter angle of Gaussian wave:
        double phi[3] = { M_PI*0.0, M_PI*0.5, M_PI*0.0 };
        ...
        for (auto h : this->hg->hexen) {
            double cosphi = (double) cos (phi[0]);
            double sinphi = (double) sin (phi[0]);
            DBG ("phi: " << phi[0]);
            double x_ = (h.x * cosphi) + (h.y * sinphi);
            this->rhoA[h.vi] = gain * exp(-((x_-xoffA)*(x_-xoffA)) / sigma);
            cosphi = (double) cos (phi[1]);
            sinphi = (double) sin (phi[1]);
            DBG ("phi: " << phi[1]);
            x_ = (h.x * cosphi) + (h.y * sinphi);
            this->rhoB[h.vi] = gain * exp(-((x_-xoffB)*(x_-xoffB)) / sigma);
            cosphi = (double) cos (phi[2]);
            sinphi = (double) sin (phi[2]);
            DBG ("phi: " << phi[2]);
            x_ = (h.x * cosphi) + (h.y * sinphi);
            this->rhoC[h.vi] = gain * exp(-((x_-xoffC)*(x_-xoffC)) / sigma);
        }
    ...
    void plot (vector<morph::Gdisplay>& disps, bool savePngs = false) {
        ...
        vector<list<Hex> > ctrs = this->get_contours (this->contour_threshold);
...
#define INDIVIDUAL_SCALING 1
        ...
        array<float,3> offset = { (N_TC/2)*(-hgwidth-(hgwidth/20)), 0.0f, 0.0f };
    ...
    void add_contour_plot (vector<list<Hex> >& contourHexes, morph::Gdisplay& disp) {
     ...
     */
    void plot_contour (vector<list<Hex> >& contourHexes, morph::Gdisplay& disp) {
