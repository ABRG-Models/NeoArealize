/*
 * 2D Karbowski system with sigmoidal normalization of a_i and
 * accumulation and competition
 */

#include "rd_james_comp10.h"

template <class Flt> //       RD_James_comp10 base class gives sigmoidal normalization
class RD_James_comp11 : public RD_James_comp10<Flt>
{
public:
    //! The accumulation parameter
    ...
    RD_James_comp11 (void)
        : RD_James_comp10<Flt>() {
    }

    virtual void allocate (void) {
        RD_James_comp10<Flt>::allocate();
        this->resize_vector_param (this->epsilon, this->N);
        this->resize_vector_vector (this->a_eps, this->N);
    }

    virtual void init (void) {
        RD_James_comp10<Flt>::init();
        cout << "y verifies: " << this->y << endl;
    }
...
#pragma omp parallel for
            for (unsigned int h=0; h<this->nhex; ++h) {
                k1[h] = this->divJ[i][h] + (this->y * this->a[i][h]* this->a[i][h]) - this->dc[i][h] - (this->a[i][h] * eps[h]);
