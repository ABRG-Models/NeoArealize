...
#pragma omp parallel for shared(i,k)
        ...
        for (unsigned int i=0; i<this->N; ++i) {
            ...
            // Idea: Do D grad (sum(a_i)) for computation of div_J
...
#pragma omp parallel for
            ...
            this->compute_divJ (a[i], sum_a_ne_i, i); // populates divJ[i]
...
#pragma omp parallel for schedule(static)
    ...
    void compute_divJ (vector<Flt>& fa, vector<Flt>& fa_others, unsigned int i) {
        ...
        // First add fa to fa_others for a summed a.
        vector<Flt> fa_sum(this->nhex, 0.0);
#pragma omp parallel for
        for (unsigned int hi=0; hi<this->nhex; ++hi) {
            fa_sum[hi] = fa[hi] + fa_others[hi];
        }
