/*
 * Competition method 5; evolution of method 4 - biggest a has no
 * attenuation.
 */
...
template <class Flt>
class RD_James_comp5 : public RD_James<Flt>
{
public:
    /*!
     ...
     */
    RD_James_comp5 (void) : RD_James<Flt>() {}
...
#pragma omp parallel for
            for (unsigned int h=0; h<this->nhex; ++h) {
                ...
                for (unsigned int j=0; j<this->N; ++j) {
                    if (this->a[j][h] > this->a[biggest][h]) {
                        biggest = j;
                    }
                }
                // Store a[i][h] * eps[h] for analysis (it's also used 4 times below)
                if (i == biggest) {
                    // No attenuation for biggest valued a at hex h
                    this->a_eps[i][h] = (Flt)0.0;
                } else {
